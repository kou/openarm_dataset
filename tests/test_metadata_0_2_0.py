...
METADATA_PATH = Path(__file__).parent / "fixture" / "dataset_0.2.0" / "metadata.yaml"
...
def test_frequencies():
    ...
    assert frequencies.action == {
        "arms": {
            "right": pytest.approx(250.0),
            "left": pytest.approx(250.0),
        }
    }
    assert frequencies.cameras == {
        "wrist_right": pytest.approx(1000.0 / 33),
        "wrist_left": pytest.approx(1000.0 / 33),
        "ceiling": pytest.approx(1000.0 / 33),
        "head": pytest.approx(1000.0 / 33),
    }
    assert frequencies.obs == {
        "arms": {
            "right": pytest.approx(250.0),
            "left": pytest.approx(250.0),
        }
